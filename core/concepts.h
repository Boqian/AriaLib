...
template <class Derived, class Base> concept derived_from =
    is_base_of_v<Base, Derived> && is_constructible_v<const volatile Derived *, const volatile Base *>;
...
template <class T> concept copy_constructible = move_constructible<T> && constructible_from<T, T &> && convertible_to<T &, T> &&
                                                constructible_from<T, const T &> && convertible_to<const T &, T> &&
                                                constructible_from<T, const T> && convertible_to<const T, T>;

template <class LHS, class RHS> concept assignable_from = is_lvalue_reference_v<LHS> && requires(LHS lhs, RHS &&rhs) {
  { lhs = forward<RHS>(rhs) } -> same_as<LHS>;
};

template <class T> concept default_initializable = constructible_from<T> && requires { T{}; } && requires { ::new T; };

template <class T> concept integral = is_integral_v<T>;

template <class T> concept signed_integral = integral<T> && is_signed_v<T>;

template <class T> concept unsigned_integral = integral<T> && is_unsigned_v<T>;

template <class T> concept floating_point = is_floating_point_v<T>;

template <class T> concept arithmetic = is_arithmetic_v<T>;

template <class T> concept _boolean_testable_impl = convertible_to<T, bool>;

template <class T> concept boolean_testable = _boolean_testable_impl<T> && requires(T &&t) {
...
template <class T, class U> concept _Use_ADL_swap =
    (has_class_or_enumTpe<T> || has_class_or_enumTpe<U>)&&requires(T &&t, U &&u) { swap(forward<T>(t), forward<U>(u)); };

struct _Cpo {
  template <class T, class U> requires _Use_ADL_swap<T, U> constexpr void operator()(T &&t, U &&u) const noexcept {
    swap(static_cast<T &&>(t), static_cast<U &&>(u));
  }

  template <class T> requires(!_Use_ADL_swap<T &, T &> && move_constructible<T> && assignable_from<T &, T>)
